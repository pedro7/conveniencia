...
def adicionar_produto(request: HttpRequest):
    ...
    try:
        produto = Product.objects.get(barcode=codigo_barras)
...
def finalizar_compra(request: HttpRequest):
    ...
    if not colaborador:
        return redirect('visualizar_carrinho')
    compra = Purchase.objects.create(colaborador=colaborador)
    ...
    for produto, quantidade in counter.items():
        ...
        compra.products.add(produto, through_defaults=through_defaults)
    carrinho.clear()
    return redirect('visualizar_carrinho')
...
def consultar_gasto_mensal(request: HttpRequest):
    ...
    for compra in Purchase.objects.filter(collaborator=colaborador, date__gte=referencia_atual):
        for compra_produto in compra.purchase_products.all():
            gasto_referencia_atual += compra_produto.unit_price * compra_produto.quantity
    gasto_referencia_passada = 0
    for compra in Purchase.objects.filter(collaborator=colaborador, date__range=[_get_referencia_passada(referencia_atual), referencia_atual]):
        for compra_produto in compra.purchase_products.all():
            gasto_referencia_passada += compra_produto.unit_price * compra_produto.quantity
    context = {
        'carrinho': carrinho,
...
def _get_colaborador_valido(request, login, senha):
    ...
    if colaborador.status == 'inativo':
        ...
        return None
    if not check_password(senha, colaborador.password):
